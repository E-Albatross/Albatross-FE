...
const SubLiter = ({ navigation, id, setTitle, text, pageNum }) => {
  ...
  const getPhotoUri = async () => {
    ...
    try {
      ...
      const getFeedback = async (JsonData) => {
        // json 넘기고 피드백 받아오기
        try {
          await axios
            .post(
              `${USER_SERVER}/score/${name}/${id}/${pageNum + 1}/${fontPath}`,
      ...
      const postServer = async () => {
        ...
        try {
          ...
          await fetch(
            `${USER_SERVER}/image/s3/resource/${userId}/${id}/${
              pageNum + 1
            }/${name}`,
            {
              method: 'POST',
          ...
          console.log(
            `https://albatross-backend.s3.ap-northeast-2.amazonaws.com/captured-image/${name}`
          );
        } catch (err) {
      ...
      const postAI = async () => {
        ...
        try {
          ...
          await fetch('http://43.131.250.175:7012/predict', {
            ...
            .then((data) => {
              if (JSON.stringify(data) === JSON.stringify(ExJson)) {
                // 빈 배열이 온다면
                setLoading(false);
                setEmpty(true);
              } else {
      ...
      {!isReady || userId == null ? (
        <>
          ...
          <AppleAuthentication.AppleAuthenticationButton
            ...
            onPress={async () => {
              try {
                const identity = await AppleAuthentication.signInAsync({
                  requestedScopes: [
        ...
        <>
          ...
          >
            <View style={styles.checkModalContainer}>
              ...
              <Image
                source={loading}
                sttle={{ width: 10, height: 10, marginTop: 50 }}
              />
            </View>
          </Modal>
          ...
          <ViewShot
            ...
            style={{
              marginTop: 70,
              width: windowWidth,
              height: windowHeight * 0.92 - 70,
            }}
          >
            <ViewShot
              ref={drawerRef}
              options={{ format: 'jpg', quality: 0.9, result: 'data-uri' }}
              style={{ width: windowWidth, height: windowHeight * 0.92 - 70 }}
            >
              {finish === false ? (
                <View
                  style={{
                    ...
                    <>
                      {markList.map((s, index) => (
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            setMarkModal(true);
                            setMarkModalText(feedbackText[s.fidx - 1].text);
                          }}
                        >
                          {s.fidx === 1 ? (
                            <Image
                              ...
                              style={{
                                ...
                                width: 30,
                                position: 'absolute',
                                left: s.x * 0.5 - 390 - 60 + 20 + 10,
                                top: 30 - 30 - 360 + s.line * 180,
                              }}
                              source={markIcon}
                            />
                          ) : (
                            <Image
                              ...
                              style={{
                                ...
                                width: 30,
                                position: 'absolute',
                                left: s.x * 0.5 - 390 - 60 + 20,
                                top: -30 - 360 + s.line * 180,
                              }}
                              source={markIcon}
                            />
                          )}
